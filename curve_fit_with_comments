# -*- coding: utf-8 -*-
"""
Created on Fri Jun 19 11:31:56 2015
...
"Temperature data for propane, given from Perkins, Ochoa, Magee's paper (2009)"
...
"Vapor pressure data for propane, given from Perkins, Ochoa, Magee's paper (2009)"           
...
def fit_func( x, a, b, c, d):
    """
    Define the fit function for the vapor pressure data based on the equation given from Dr. Magee
    Variables: 
    tc-critical temperature(K) 
    pc-critical pressure (MPa) 
    (4.26096 pc and 369 tc for propane)
    """
    ...
    #q = a*np.exp(b*x)+c  
#%% 
#utilize scipy's curve_fit function to curve fit the data given above
#to the defined fit function and print the defined popt array
popt, pcov = curve_fit(fit_func, xdata, ydata)
print popt
"""
Calculates the SSE, SST, and R-squared value for our curve fit
Variables:
sse - exponential sum of squares (?)
...
#%%
"Print title, create a grid for plot, label axes, and graph the curve and data points"
plt.title('Vapor Pressure vs Temperature of Substance')
plt.grid(True)
